...
def era_to_gregorian(label: str) -> int:
    ...
    # Fallback: try plain year number in the label.
...
def normalize_category(raw: Optional[str]) -> str:
    ...
    if "sec" in text:
        return "security"
    if "db" in text or "data" in text:
        return "database"
    if "manage" in text or "project" in text:
        return "management"
    if "ネット" in raw or "network" in text:
        return "network"
    if "セキュ" in raw:
        return "security"
    if "データ" in raw or "データベース" in raw:
        return "database"
    if "マネジ" in raw or "ストラテジ" in raw:
        return "management"
...
def extract_questions_from_html(html: str, session: SessionMeta) -> List[Dict[str, Any]]:
    ...
    Adjust selectors according to the site's DOM.
    """
    soup = BeautifulSoup(html, "html.parser")
    normalized: List[Dict[str, Any]] = []

    # AP-Siken specific parsing (single question per page).
    ...
    if siken_question:
        ...
        raw_cat = None
        bunya = soup.find("h3", string=lambda s: s and "分野" in s)
        if bunya:
            info_div = bunya.find_next("div")
            raw_cat = info_div.get_text(" ", strip=True) if info_div else None
